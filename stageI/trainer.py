...
class CondGANTrainer(object):
    ...
    def epoch_sum_images(self, sess, n):
        ...
        print(gen_samples[0])
        imageio.imwrite('%s/train.jpg' % (self.log_dir), (gen_samples[0] * 255).round().astype(np.uint8))
        imageio.imwrite('%s/test.jpg' % (self.log_dir), (gen_samples[1] * 255).round().astype(np.uint8))
    ...
    def save_super_images(self, images, sample_batchs, filenames, sentenceID, save_dir, subset):
        ...
        for j in range(len(filenames)):
            ...
            print(superimage)
            arr_min = np.min(superimage)
            arr_max = np.max(superimage)
            superimage = (superimage - arr_min) / (arr_max - arr_min)
            superimage = (superimage * 255).round().astype(np.uint8)
            print(superimage)
